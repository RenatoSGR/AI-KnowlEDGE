...
def start_docker_compose(yml_path: str) -> str:
    try:
        ...
        return subprocess.run(["docker-compose", "-f", str(yml_file), "up", "-d"], check=True)
...
def main():
    ...
    document_file = "../document/one_page_test.pdf"

    status = start_docker_compose(yml_path)
    if status == 0:
        endpoint = get_endpoint_from_docker_compose(yml_path)
        key = get_key(azure_env_path)
        client = get_document_intelligence_client(endpoint, key)

        start = time.time()
        result = analyze_document(client, document_file)
        elapsed = time.time() - start

        document_info = get_results(result)
    else:
        document_info = {"error": status}
    # Process document_info as needed
